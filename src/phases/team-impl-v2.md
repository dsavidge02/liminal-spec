# Team Implementation Orchestration — External CLI (v2 — Experimental)

**⚠️ Experimental skill.** This is `ls-team-impl-v2`, paired with `ls-tech-design-v2`. It mirrors the stable `ls-team-impl` body and adds conditional handling for an optional `ui-spec.md` companion produced by the v2 tech design. Use `ls-team-impl` for the stable workflow. See `docs/research/ls-tech-design-v2-plan.md` for the experiment plan, decision matrix, and exit criteria.

The skill stays a generic external-CLI orchestration skill (Codex or Copilot user-selectable). The v2 pilot will *select Codex* at the CLI-selection step, but the skill itself preserves v1's CLI selection behavior so the experiment isolates the UI-spec companion as the only meaningful variable.
...
You receive technically enriched stories (and optionally an epic, a tech design, and a UI spec). You implement them sequentially, one story at a time. Teammates manage the external CLI; you manage the process.
...
Your job:
- Read and understand the stories, epic, tech design, and (if present) UI spec before starting
...
- **UI spec** (optional, v2-specific) — `ui-spec.md` produced by `ls-tech-design-v2` when the UI Companion Invocation Rubric fired. Present for frontend-heavy features. Carries screen inventory, AC-to-screen/state map, component specifications referencing tech-design identifiers, and the project's chosen verification surface (typically Playwright screenshot capture).

Epic, tech design, and UI spec are optional because stories may exist without those documents. The story is always required.
...
Once the human provides paths, read the epic, tech design, and (if present) UI spec. List the stories and read the first story. If there are multiple stories, read the first two to understand the dependency relationship and scope progression.

Understanding the cross-cutting patterns in the tech design is the most important preparation. The stories contain the what (ACs, TCs). The tech design contains the architecture context that ties everything together — how modules interact, why decisions were made, what the flow patterns are. **When a UI spec is present, read it after the tech design and before the epic** — it annotates the frontend surface of the ACs and tells you which screens, states, and components the implementation has to honor visually. An orchestrator who only reads stories will miss the connections that inform good handoff decisions.
...
Step 2 — Read artifacts sequentially, reflecting after each one:
  ...
  5. [test plan if available] — Read. Reflect: what testing patterns and coverage
     expectations apply?
  6. [story] — Read. Reflect: what are the ACs, TCs, and any spec deviations or
     gotchas to flag?
...
Step 2 — Read artifacts sequentially, reflecting after each one (same as implementer):
  ...
  before reading the next. Include the UI spec (if present) before the epic. Include
  the story as the final read. Write cumulative reflections to
  /tmp/reflection-review-story-N.md before starting the review.
...
Step 3 — Dual review (parallel):
  A. Launch CLI for spec-compliance review. Give it artifact paths and instruct:
     ...
     If a UI spec is present, the CLI must additionally check:
       - Components named in the UI spec are present in the implementation
       - State presentations specified in the UI spec are reachable in code
...
Re-read the materialized implementer template from the log. Construct the handoff as one complete prompt (not drip-fed). Substitute the actual paths and any story-specific flags you noticed while reading — like "this story has a spec deviation worth noting" or "this is a large story, ~40 tests expected." When a UI spec is in scope for the story, include its path in the artifact list and flag any specific screens/states the story owns.
...
4. UI spec compliance summary (if applicable): structural checks, screenshot artifact paths, one-way ownership contract status, human visual review status
5. Open risks (or `none`)
...
- Boundary inventory
- UI spec compliance state — if Story 2 produced screenshots for Screen A, Story 3's handoff notes that and flags new screens this story owns
...
2. **Reflect against the epic, tech design, and UI spec.** The artifacts contain the rationale for decisions. Most questions can be answered by tracing back to the spec.
...
Launch four reviewers simultaneously. Each reads the full epic, the full tech design, the UI spec (if present), and the entire codebase. Each writes a detailed review report to their designated directory.

**Two Claude teammates** (general-purpose, not senior-engineer):

1. **Opus reviewer** — reads epic, tech design, UI spec, all source files, all test files. Writes `epic-review.md` to their directory.
...
- Read the epic (path), the tech design (path), the UI spec (path, if present), and every source and test file in the project
- Specify the exact working directory for CLI reviewers
- Do a thorough critical review of the full implementation against the epic, tech design, and UI spec
...
- If a UI spec is present: verify cross-story visual consistency, component reuse compliance, complete state coverage across all screens, screenshot artifacts present and consistent across the epic
...
6. **If a UI spec is present:** include a final pass surfacing the screenshot set for human visual review. Structural compliance is verified by the agents; visual quality remains a human gate.

### Phase 4: Fixes

After discussion and human approval:

- Launch a teammate with the CLI to implement the approved fixes. Give them the fix list file, the epic, the tech design, and the UI spec (if present).
...
Agents that read the full epic + tech design + UI spec + story, implement multiple modules, and then process review feedback can exhaust their context window. Symptoms: the agent goes idle without completing, or produces truncated/confused output.
...
### UI Spec Verification Ceiling (v2-specific)

When a UI spec is present, agents and CLI reviewers verify *structural* UI compliance only — components named in the spec exist in code, named states are reachable, tech-design identifiers resolve cleanly, and the verification surface (Playwright screenshots by default) is produced. Agents cannot reliably judge visual polish, spacing rhythm, hierarchy, or hover-state feel from code alone.
...
- Human visual review of UI screenshots before story or epic acceptance when a UI spec was in scope
...
- UI spec presence (path if present, or `none`) and the verification surface chosen by the spec
...
- UI spec compliance state and screenshot artifact paths when applicable
