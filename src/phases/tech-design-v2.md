# Tech Design (v2 — Experimental)

**⚠️ Experimental skill.** This is `ls-tech-design-v2`, paired with `ls-team-impl-v2`. It layers an optional UI companion artifact on top of the stable Tech Design contract. Use `ls-tech-design` for the stable workflow. See `docs/research/ls-tech-design-v2-plan.md` for the experiment plan, decision matrix, and exit criteria.

**Purpose:** Transform Epic into Tech Design with architecture, interfaces, and test mapping. When the feature is frontend-heavy, also produce a `ui-spec.md` companion that captures visual, state, and interaction intent that the behavioral ACs alone do not carry.
...
### Output Contract Addendum (v2 only)

`ui-spec.md` is a **separate artifact class**, not part of the Tech Design 2-doc / 4-doc configuration count. The "Never go 3" rule stays intact because `ui-spec.md` is not one of the `tech-design*` docs — it is a UI companion that lives alongside the Tech Design output, sibling to it.

- Config A + UI companion produces: `tech-design.md`, `test-plan.md`, `ui-spec.md` (3 files total, but Tech Design is still 2 docs).
- Config B + UI companion produces: `tech-design.md`, `tech-design-[domain-a].md`, `tech-design-[domain-b].md`, `test-plan.md`, `ui-spec.md` (5 files total, but Tech Design is still 4 docs).

The UI companion is produced **only when invoked by the rubric below**. It is optional; backend-only or non-UI epics do not produce it, and v2 behaves identically to v1 in that case.
...
- UI companion (`ui-spec.md`) if any trigger in the UI Companion Invocation Rubric fires
...
- UI companion (`ui-spec.md`) — conditional; produced only when the UI Companion Invocation Rubric below fires
...
This section is unique to `ls-tech-design-v2`. When the invocation rubric fires, you produce a `ui-spec.md` companion alongside the Tech Design output. When it does not fire, skip the companion and proceed as normal — v2 behaves identically to v1 in that case.

## UI Companion Invocation Rubric

Produce `ui-spec.md` when **any** of the following is true. Three triggers, not six — the rubric stays crisp so invocation is consistent.

1. **Net-new screens or major screen re-composition.** The feature introduces screens that do not currently exist, or materially restructures existing screens' layout.
2. **New or materially changed reusable UI primitives or token families.** The feature introduces shared components, layout shells, spacing rhythms, color roles, or typography tokens that other features will also consume.
3. **Reference materials were provided.** The user supplied mockups, screenshots, Figma exports, sketches, or other visual artifacts that must be honored.

If none fire, the companion is not produced and nothing in this section applies. If any fire, produce the companion per the sections below.

## One-Way Ownership Contract

The UI spec and the Tech Design sit side by side. To prevent dual-source-of-truth drift, ownership is one-way:
...
- **UI companion references tech design identifiers** (component name, prop symbol) rather than redefining them.

If the UI companion is redefining an interface, it has crossed the line. Rewind and reference instead.

## Confidence Chain Relationship

The authoritative chain stays `AC → TC → Test → Implementation`. The UI companion **annotates** the frontend surface of ACs — it maps ACs to screens and states — but it does not replace ACs as the upstream authority and it does not extend the chain. Traceability flows `AC → screen/state` inside the companion; the ACs themselves remain in the epic.

## Process Phases

### Phase 1: Reference Material Review (conditional)

If reference materials were provided:
- Analyze each artifact for layout patterns, color usage, component structure, interaction hints, and visual direction.
...
If no references were provided, skip this phase.

### Phase 2: Design Inventory (codebase-aware)

Scan the codebase for existing UI patterns:
- Shared components or design-system primitives
...
If `ls-current-docs` output exists in the project, use it as a head start — the inventory here is narrower and more design-focused, but the codebase scan overlaps.

### Phase 3: Visual System Strategy

Commit to one of two strategies for v2. (The original three-way strategy included an Establish path; v2 narrows that path to gap documentation — see below.)

- **Inherit** — a mature, consistent design system exists. Spec screens using existing tokens and components. Propose additions only where real gaps exist.
...
**Establish — narrowed to gap documentation.** If the project has no meaningful UI system, the companion **documents minimum required additions and visible system gaps**. It does **not** attempt to define a full design system (token taxonomy, complete component library, type scale). Full design-system definition is a separate, larger activity — potentially a future `ls-design-system` skill. For v2, Establish produces a gap log and the starter set the first feature needs, nothing more.

The strategy must include evidence (what was checked, what was found) and rationale. This gate prevents the skill from reinventing a system that already works or over-building in projects that don't yet have one.
...
Produce the core content of the companion:
- Screen inventory — what screens/views exist for this feature
- Flow-to-screen mapping — which epic flows map to which screens
- AC-to-screen/state map — which ACs are satisfied by which screen in which state
- Per-screen state coverage — loading, empty, error, success, validation, disabled, responsive variants
- Component specifications — new and extended components, each referencing its tech-design identifier and describing its visual structure, state presentations, and layout constraints (no interface redefinition)
...
- Design tokens are either existing (verified in codebase) or new (defined in the companion)
...
- Reference material fidelity verified — companion honors the reference or documents deviations with rationale
- One-way ownership contract respected — the companion references tech-design identifiers, does not redefine them
- Verification surface is specified (see next section)

## Verification Surface Requirement

The companion must specify **how implementation will be verified against it**. The skill does not mandate a tool; the project chooses. Default for v2 pilots: **Playwright screenshot capture for every named state**.

Rationale for the default:
- Once the Playwright harness exists, marginal cost per state is low.
...
## Output Artifact: `ui-spec.md`

A single file with these sections:

1. **Reference Material Analysis** (conditional — omitted if no references)
2. **Visual System Strategy** — Inherit / Extend / narrowed-Establish decision with evidence and rationale
...
7. **Component Specifications** — new and extended components, each referencing its tech-design identifier; visual structure, state presentations, layout constraints, token usage, accessibility expectations, and any motion/interaction notes folded in
...
4. **Trace every screen and state back to ACs and TCs** — nothing in the companion exists without a requirement link.
5. **Document every new reusable primitive as an explicit proposal** — not a silent addition.
